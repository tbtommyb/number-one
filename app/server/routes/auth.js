...
var auth = {
	...
	login: function(req, res) {
		var candidateUser = new user.Create({
	        name: req.query.name || '',
	        password: req.query.password || '',
	    });

	    if (candidateUser.name == '' || candidateUser.password == '') {
	    	 res.status(400).send({
	            success: false,
	    ...
	    user.get(candidateUser.name, req, function (err, foundUser) {
	...
	checkPassword: function(candidateUser, foundUser, callback) {
		Bcrypt.comparePassword(candidateUser.password, foundUser.password, function (err, isMatch) {
            if (err) {
                return callback(err);
            }
            if (!isMatch) {
